...
let gameResultListReducer = Reducer<GameResultListState,
                                    GameResultListAction,
                                    GameResultListEnvironment> {
                                        state, action, env in
                                        switch action {
...
struct GameResultListView_Previews: PreviewProvider {
    static var previews: some View {
        GameResultListView(store:
                .init(initialState:.init(uniqueElements: [
            GameResult(counter: .init(count: 20, secret: 20, id: .init()),
                       timeSpent: 100),
